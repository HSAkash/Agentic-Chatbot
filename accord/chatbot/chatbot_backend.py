...
from accord.agent_graph.build_full_graph import build_graph
from accord.utils.common import create_directory, get_config
from accord.utils.memory import Memory
from pyprojroot import here


PROJECT_CFG = get_config(here("configs/project_config.yaml"))
TOOLS_CFG = get_config(here("configs/tools_config.yaml"))

graph = build_graph()
config = {"configurable": {"thread_id": TOOLS_CFG.graph_configs.thread_id}}
...
class ChatBot:
    ...
    def respond(chatbot: List, message: str) -> Tuple:
        ...
        Memory.write_chat_history_to_file(
            ...
            folder_path=PROJECT_CFG.memory.directory,
            thread_id=TOOLS_CFG.graph_configs.thread_id
        )
        return "", chatbot
